...
# Pre-compiled patterns used across multiple methods
...
class SubtitleAgent:
    ...
    def reformat_with_ai(
        ...
        # Build transcript text with [PAUSE] markers at silence gaps
        transcript_with_pauses = self._build_transcript_with_pauses(all_words)

        # Ask AI to add punctuation (chunked for long transcripts)
        client = OpenAI(api_key=api_key)
        punctuated_text = self._punctuate_with_ai(transcript_with_pauses, client, model)

        # Map punctuated words back to original word timestamps
        print(f"  Mapping punctuated text to {len(all_words)} word timestamps...")
        self._map_punctuation_to_words(punctuated_text, all_words)

        # Rebuild as a single combined segment, then split at sentence boundaries
    ...
    def reformat_as_sentences(self, segments: List[TranscriptSegment]) -> List[TranscriptSegment]:
        """Split segments at sentence boundaries and pauses.
        ...
        for i, w in enumerate(all_words):
            ...
            ends_sentence = bool(_SENTENCE_END.search(word_text))
            has_pause = False
    ...
    # ------------------------------------------------------------------
    # AI punctuation helpers (extracted from reformat_with_ai)
    # ------------------------------------------------------------------

    @staticmethod
    def _build_transcript_with_pauses(
        all_words: List[WordTiming], pause_threshold: float = 0.5
    ) -> str:
        """Build transcript text with [PAUSE] markers at silence gaps."""
        text_parts: List[str] = []
        for i, w in enumerate(all_words):
    ...
    @staticmethod
    def _punctuate_with_ai(transcript: str, client: OpenAI, model: str) -> str:
        """Send transcript chunks to AI for punctuation, return combined result."""
        chunks = SubtitleAgent._split_for_api(transcript, max_chunk_chars=2000)
    ...
    @staticmethod
    def _map_punctuation_to_words(
        punctuated_text: str, all_words: List[WordTiming]
    ) -> None:
        """Walk punctuated words and originals in parallel, copying punctuation.

        If the AI changed a word, skip it rather than cascading errors.
        ...
        while p_idx < len(punctuated_words) and w_idx < len(all_words):
            ...
            p_clean = _STRIP_PUNCT.sub("", p_word).lower()

            if not p_clean:
                p_idx += 1
                continue

            w_clean = _STRIP_PUNCT.sub("", all_words[w_idx].get("word", "")).lower()
            ...
            if p_clean == w_clean:
                ...
                for look_w in range(w_idx + 1, min(w_idx + 5, len(all_words))):
                    if _STRIP_PUNCT.sub("", all_words[look_w].get("word", "")).lower() == p_clean:
                        w_idx = look_w
                        all_words[w_idx]["word"] = p_word
                        p_idx += 1
                        w_idx += 1
                ...
                if not found:
                    ...
                    for look_p in range(p_idx + 1, min(p_idx + 5, len(punctuated_words))):
                        lp_clean = _STRIP_PUNCT.sub("", punctuated_words[look_p]).lower()
                        if lp_clean == w_clean:
                            p_idx = look_p
                ...
                if not found:
                    # Can't re-sync — keep original word, skip AI word
                    p_idx += 1
    ...
    def _flatten_to_words(segments: List[TranscriptSegment]) -> List[WordTiming]:
        ...
        for seg in segments:
            if seg.words:
                ...
                # Fallback: estimate word timing from segment duration
